...
def get_spicy_food_by_cuisine(spicy_foods, cuisine)
  searched_array = spicy_foods.find { |food| food[:cuisine] == cuisine}
...
def sort_by_heat(spicy_foods)
  sorted_array = spicy_foods.sort_by { |food| food[:heat_level]}
end
...
def average_heat_level(spicy_foods)
  ...
  spicy_foods.select do |food| 
    heat_levels_array << food[:heat_level]
  end
  heat_levels_array.sum(0.0) / heat_levels_array.size
end
